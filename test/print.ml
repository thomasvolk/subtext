...
let s_option o = match o with None -> "None" | Some s -> "Some(" ^ s ^ ")"

let note n =
  "Note(slug="
  ^ Note.Slug.to_string (Note.slug n)
  ^ ", text=" ^ Note.text n ^ ")"
...
let reference_tuple (k, r, kd) =
  Note.Slug.to_string k ^ ", " ^ r ^ ", " ^ reference_kind kd
...
let reference r =
  ...
  let kind =
    match r.kind with SlashLink -> "SlashLink" | WikiLink -> "WikiLink"
  in
...
let action a =
  ...
  match a with
  | RenameNote (o, n) ->
      "RenameNote(" ^ Note.Slug.to_string o ^ ", " ^ Note.Slug.to_string n ^ ")"
...
let batch_list l =
  String.concat " "
    (List.map
       (fun (a, b) -> Note.Slug.to_string a ^ "->" ^ Note.Slug.to_string b)
       l)
