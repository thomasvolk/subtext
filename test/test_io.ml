...
let tests base_dir =
  let ror =
    Io.FileRepository.create ~base_dir ~file_extension:"subtext" ~read_only:true
  in
  let rwr =
    Io.FileRepository.create ~base_dir ~file_extension:"subtext"
      ~read_only:false
  in
         ...
         ( "read notes" >:: fun _ ->
           assert_equal ~printer:Print.note_list
             [
               ...
               Note.create "foo"
                 "This is the foo note see also [[ sub // bar ]]";
...
|};
         ...
         ( "rename" >:: fun _ ->
           ...
           assert_bool
             "file A/B/C/1.subtext exists but dry-run mode was enabled"
             (not (Sys.file_exists new_note_path));

           ST.rename rwr "1" "A/B/C/1";

           assert_bool "file A/B/C/1.subtext does not exists"
             (Sys.file_exists new_note_path) );
         ( "rename to an existing file" >:: fun _ ->
           let expected =
             Subtext.Exn.InterruptExecution
               ("rename fails - file already exists: "
               ^ Filename.concat base_dir ("foo" ^ ".subtext"))
           in
