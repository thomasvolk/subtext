...
let tests =
  ...
  >::: [
         ( "Rename.command" >:: fun _ ->
           let open Op in
           let open Note in
           assert_equal ~printer:Print.command Stop
             (Rename.command
                [ Note.create "old" "  /old   /a  /b " ]
                (Slug.create "not-found") (Slug.create "new"));

           assert_equal ~printer:Print.command Stop
             (Rename.command [] (Slug.create "old") (Slug.create "new"));

           assert_equal ~printer:Print.command
             (Action
                ( RenameNote (Slug.create "old", Slug.create "new"),
                  Action
                    (WriteNote (Slug.create "new", "  /new   /a  /b "), Stop) ))
             (Rename.command
                [ Note.create "old" "  /old   /a  /b " ]
           ...
           assert_equal ~printer:Print.command
             (Action
                ( RenameNote (Slug.create "old", Slug.create "1-a/new"),
                  Action
                    ( WriteNote
                        (Slug.create "foo", "  /1-a/new test [[ 1 a//new ]] "),
                      Action
                        ( WriteNote
                            (Slug.create "1-a/new", "  /1-a/new   /a  /b "),
                          Stop ) ) ))
             (Rename.command
                [
                  Note.create "old" "  /old   /a  /b ";
         ...
         ( "parse_pattern" >:: fun _ ->
           let nl = [ Note.create "foo/1" ""; Note.create "foo/2" "" ] in
         ...
         ( "generate graph" >:: fun _ ->
           ...
           in
           assert_equal ~printer:Print.string
             {|digraph "test" {
"a" -> "a";
"a" -> "x";
"b" -> "a";
