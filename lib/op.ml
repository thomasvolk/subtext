...
let string_of_action =
  ...
  | RenameNote (o, n) ->
      "rename note form " ^ to_string o ^ " to " ^ to_string n
...
module Rename = struct
    ...
    let rec update_chain notes next =
      ...
      | h :: t ->
          ...
          update_chain t
            ...
            | None -> next
            | Some t ->
                let k = if kcur = kold then knew else kcur in
  ...
  module Batch = struct
    module Source = struct
      type t = All | StartsWith of string

      let of_string = function "." -> All | s -> StartsWith s
    end

    module Target = struct
      type t = RemovePrefix | AddPrefix of string

      let of_string = function "." -> RemovePrefix | s -> AddPrefix s
    end
    ...
    let parse_pattern s t nl =
      ...
      | All, RemovePrefix -> []
      | All, AddPrefix p -> slugs |> List.map (fun s -> (s, add_prefix p s))
      | StartsWith b, AddPrefix p ->
          slugs
          |> List.filter (fun k ->
                 String.starts_with ~prefix:b (Note.Slug.to_string k))
          |> List.map (fun s -> (s, add_prefix p s))
      | StartsWith b, RemovePrefix ->
          slugs
          |> List.filter (fun k ->
                 String.starts_with ~prefix:b (Note.Slug.to_string k))
  ...
  module Make (R : Repository) = struct
    ...
    let rename r o n = rename_note r (Note.Slug.create o) (Note.Slug.create n)

    let batch_rename r s t =
      let nl = R.read_notes r in
      Batch.parse_pattern s t nl |> List.iter (fun (o, n) -> rename_note r o n)
  end
end

module DotGraph = struct
  let to_edge a b =
    "\"" ^ Note.Slug.to_string a ^ "\" -> \"" ^ Note.Slug.to_string b ^ "\";"

  let edges n =
    List.map
      (fun r -> to_edge (Note.slug n) (Note.Reference.to_slug r))
      (Note.Reference.parse (Note.text n))

  let create name nl =
    let e = List.map edges nl |> List.flatten |> String.concat "\n" in
    "digraph \"" ^ name ^ "\" {\n" ^ e ^ "\n}"
end
