...
module FileRepository = struct
  ...
  let split p =
    Str.split (Str.regexp Filename.dir_sep) p |> List.filter (fun l -> l != "")
  ...
  type file_stat = Directory | File | NotFound
  ...
  let mkdirs p perm =
    let rec mkdir b = function
      ...
      | h :: tl -> (
          ...
          | File ->
              raise
                (Exn.InterruptExecution
                   ("can not create directory: " ^ p ^ " - " ^ d ^ " is a file"))
          )
    in
    mkdir "" (split p)

  let move_file s t =
    if file_exists t then
      raise
        (Exn.InterruptExecution ("rename fails - file already exists: " ^ t))
    else
  ...
  let chop_base_dir r p =
    let cnt = List.length (split r.base_dir) in
  ...
  let create ~base_dir ~file_extension ~read_only =
    let file_ext =
      if String.length file_extension = 0 then "subtext" else file_extension
    in
    { file_extension = file_ext; base_dir; read_only }
  ...
  let execute r a = if not r.read_only then execute_action r a
end
...
module LoggingFileRepository = struct
  ...
  let create ~base_dir ~file_extension ~read_only =
    Format.printf "init repository: dir=%s, extension=%s, write=%b\n" base_dir
      file_extension (not read_only);
    FileRepository.create ~base_dir ~file_extension ~read_only
