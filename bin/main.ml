...
let file_extension_flag = let open Command.Param in flag "-e" (optional_with_default "subtext" string) ~doc:" note file extension"
...
let () = 
  try
    Command_unix.run ~version:"0.1" ~build_info:"subtext" main_command
  with 
    Exn.InterruptExecution e -> prerr_endline ("ERROR: " ^ e);
    exit 1
